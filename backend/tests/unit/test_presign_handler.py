...
def test_returns_200_with_upload_url_and_s3_key(aws_setup):
    result = _call(aws_setup, {"content_type": "application/pdf"})
...
def test_each_call_generates_unique_s3_key(aws_setup):
    r1 = json.loads(_call(aws_setup, {"content_type": "application/pdf"})["body"])
    r2 = json.loads(_call(aws_setup, {"content_type": "application/pdf"})["body"])
...
def test_non_pdf_content_type_returns_400(aws_setup):
    result = _call(aws_setup, {"content_type": "application/msword"})
